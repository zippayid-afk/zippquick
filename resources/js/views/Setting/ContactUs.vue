<template>
    <div>
        <div class="page-heading">
            <div class="page-head">
                <h3 class="page-head-title">{{ __('contact_us') }}</h3>
                                ...
                                <b-tabs v-model="activeLanguageTab" content-class="mt-3"
                                    ...
                                    <b-tab v-for="language in languages" :key="language.id" lazy>
                                        ...
                                        <div class="form-group">
                                            ...
                                            <editor :key="'editor_contact_us_' + language.id"
                                                v-model="translations[language.id].contact_us" :init="tinymceInit"
...
export default {
    ...
    methods: {
        ...
        viewUrl(langCode = '') {
            return `${this.$baseUrl}/contact-us?lang=${langCode}`;
        },
        ...
        initTranslations() {
            ...
            this.languages.forEach(lang => {
                t[lang.id] = { contact_us: "" };
            });
            this.translations = t;
        },

        getContactUs() {
            axios.get(this.$apiUrl + "/contact_us").then(res => {
                ...
                if (this.record) {
                    try {
                        ...
                        this.languages.forEach(lang => {
                            if (parsed[lang.code]) {
                                this.translations[lang.id].contact_us = parsed[lang.code];
                            }
                        });
                    } catch {
                        if (this.defaultLanguageId) {
                            this.translations[this.defaultLanguageId].contact_us = this.record.value;
                        }
                    }
                }
        ...
        saveRecord() {
            ...
            const dataByLang = {};
            this.languages.forEach(lang => {
                dataByLang[lang.code] = this.translations[lang.id].contact_us || "";
            });

            axios.post(this.$apiUrl + "/contact_us/save", {
