            ...
            <MazerDatatable responsive
                ...
                <template #cell(actions)="row">
                    <div class="list-actions">
                        ...
                        <button v-if="!nonDeleteAbleRoles.includes(row.item.name) && row.item.name !== $roleNameStore" class="list-action-btn is-delete" @click="deleteRecord(row.index,row.item.id)" v-b-tooltip.hover :title="__('delete')">
                            <Trash2 :size="15" />
                        </button>
                    </div>
                </template>
...
export default {
    ...
    data: function() {
        return {
            roles: [],
            nonDeleteAbleRoles : [this.$roleSuperAdmin,this.$roleDeliveryBoy],
            isLoading: false,
            edit_record : null,
            create_new : null,
            fields: [
